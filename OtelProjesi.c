...
typedef struct{
	...
	int odaNumarasi;//1-100 arasında.
	double fiyatBilgisi;
...
void yeniMusteriEkleme(odaBilgileri *odalar){
	...
	odalar[sorgulanacakOda-1].odaDurumu=1;// yani odamız doldu.
}
...
int main(){
	...
	odaDizisi=(odaBilgileri*)calloc(ODA_SAYISI,sizeof(odaBilgileri));//dinamik şekilde oluşurduk.
		...
		switch(ch){
			...
			case 'c':
				if(kullaniciYetkisi==personel){
					...
					scanf("%d",&odaNumarasi);
					rezervasyonTemizle(odaDizisi,odaNumarasi);//daha onceden kulllandıgımız rezervasyontemizle fonksiyonu.
				}
				else{
					printf("Bu alan sadece calisanlar tarafindan kullanilabilir.\n");
				}
